...
const WeatherFeed = () => {
  ...
  const lastSevenRounds = historyRounds.slice(-7);
        ...
        <CardContent>
          ...
          {lastSevenRounds.length > 0 ? (
            <Box sx={{ display: 'flex', alignItems: 'flex-end', height: 180 }}>
              {(() => {
                const values = lastSevenRounds.map(r => (typeof r.rainfall === 'number' ? r.rainfall : 0));
                const maxValue = Math.max(1, ...values);
                return lastSevenRounds.map((r) => {
                  const val = typeof r.rainfall === 'number' ? r.rainfall : 0;
                  ...
                  return (
                    <Box key={r.roundId} sx={{ flex: 1, px: 0.5 }}>
                      <Box
                        ...
                        title={`Round ${r.roundId}: ${val.toFixed(2)}mm`}
                      >
